...
class Brands extends CI_Controller
{
	...
	if($this->core_model->admin_can())
	{
		...
		if($this->form_validation->run() != false )
		{
		    ...
		    $ret= $this->pages_model->add_pages($title, $content, '',$active,$pic,$thumbnail, 'brands') ;
		    $data['inserted']=$ret;
		}
	    }
		...
		if($this->core_model->admin_can())
		{
			if($this->input->post('edit_news'))
			{
				...
				if($up)
				{
				    ...
				    $this->pages_model->edit_pages($title, $content, '',$active,$pic,$thumbnail, $id,array("icon"=>$this->input->post('icon')), serialize( array("coinName"=>$this->input->post('coinName') , "coinCode"=>$this->input->post('coinCode') ) ) );
				    $data['updated']=true;
				}
                }else{
					show_404();
			...
			$data['has'] = $this->has;
			$data['admin_dir']= base_url().$this->core_model->admin_dir();
			$data['page_title'] = "تعديل ماركة \"". $data['row']['title'] ."\"";
			$this->load->view( $this->core_model->admin_dir().'/templates/header', $data);
			$this->load->view( $this->core_model->admin_dir().'/'.$this->cont.'/add', $data);
			$this->load->view( $this->core_model->admin_dir().'/templates/footer', $data );
		}else{
