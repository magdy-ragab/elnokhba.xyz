...
class Gallery extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->titles = array('إضافة للقائمة', 'عرض القائمة');
	...
	public function add()
	{
		if($this->core_model->admin_can())
		{
			...
			{
				$this->form_validation->set_rules('title', 'عنوان الصورة ', 'required',
					array('required' => '- لم تقم بكتابة %s.'));
				/*$this->form_validation->set_rules('up', ' الصورة ', 'required',
					array('required' => '- لم تقم برفع %s.'));*/
				...
				if($this->form_validation->run() != false) {
					...
					}else
					{
						...
						if($up)
						{
							$info= getimagesize($_FILES['up']['tmp_name']);
							$size=$_FILES['up']['size'];
							...
							else
							{
								...
								$meta= array("w"=>$info[0], "h"=>$info[1], "type"=>$info['mime'], "size"=>$this->core_model->formatSizeUnits($size));
								$ar=array("price"=>$this->input->post('price'), "star"=>$this->input->post('star'));
								$ret= $this->pages_model->add_pages($title, $content, $news_date,$active,$pic,$thumbnail, $this->cont, serialize($meta), $size, $ar );
								$data['inserted']=$ret;
							}
						}else{
							$data['no_upload']=1;
		...
		$data['page_title'] = "إضافــة للقائمة";
	...
	public function edit($slug)
	{
		if($this->core_model->admin_can())
		{
			...
			$data['has'] = $this->has;
			$data['admin_dir']= base_url().$this->core_model->admin_dir();
			$data['page_title'] = "تعديل القائمة \"". $data['row']['title'] ."\"";
			$this->load->view( $this->core_model->admin_dir().'/templates/header', $data);
			$this->load->view( $this->core_model->admin_dir().'/'.$this->cont.'/add', $data);
			$this->load->view( $this->core_model->admin_dir().'/templates/footer', $data );
		}else{
