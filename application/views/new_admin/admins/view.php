...
<div class="row">
	...
	<div class="col-md-2 hidden-xs">
		<a href="#" role="button" class="btn btn-primary btn-block" class="dropdown-toggle"
		 data-toggle="dropdown">المشرفين <span class="caret"></span></a>
	...
	<table class="table table-bordered table-striped table-hover">
		<thead>
			<tr>
				...
				<th class="col-xs-6">الاسم</th>
				<th class="col-xs-2">الرتبة</th>
				<th class="col-xs-3">إجراءات</th>
			</tr>
		</thead>
		<tbody>
			<?php
			foreach ($this->core_model->admins(array("module"=>$cont)) as $row)
			{
			?>
			<tr>
				...
				<b><?php echo $row->username;?></b><br /><?php echo $row->email;?> 
				</td>
				<td><?php echo ($row->mode=='super')?'مدير':'مشرف';?>
				</td>
				<td class="text-center">
					...
					<a href="<?php echo $admin_dir; ?>/<?php echo $cont?>/edit_password/<?php echo $row->ID ?>" role="button" class="btn btn-xs btn-primary"><span class="glyphicon glyphicon-lock"></span> كلمة المرور</a>
					<button type="button" data-controller="<?php echo $cont?>" class="delCat btn btn-danger btn-xs" data-toggle="modal" data-target="#deleteModel" data-id="<?php echo $row->ID?>" data-title="<?php echo $row->username?>"><span class="glyphicon glyphicon-trash"></span> حذف</button>
					</div>
				</td>
			</tr>
			<?php
