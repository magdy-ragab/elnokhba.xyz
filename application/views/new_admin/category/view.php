	...
	<table class="table table-bordered table-striped table-hover">
		<thead>
			<tr>
				...
				<th class="col-xs-2">المنتجات</th>
				<th class="col-xs-2">إجراءات</th>
			</tr>
		</thead>
		<tbody>
			<?php
			foreach ($this->core_model->pages(array("module"=>$cont,"parent_id"=>0)) as $row)
			{
			?>
			<tr>
				...
				<span data-toggle="tooltip" title="حقول القسم التي تظهر عند اضافة منتجات به" data-placement=bottom> <a href="<?php echo base_url()."new_admin/{$cont}/fields/{$row->ID}" ?>"><i class="glyphicon glyphicon-sort-by-attributes-alt gray "></i></a></span>
				<?php /* <span data-toggle="tooltip" title="الفئات" data-placement=bottom> <a href="<?php echo base_url()."new_admin/{$cont}/brands/{$row->ID}" ?>"><i class=" glyphicon glyphicon-heart gray "></i></a></span> */ ?>
			...
			<?php
			$this->core_model->getSubCats($cont,$row->ID,$admin_dir);
			}
			?>
		</tbody>
	</table>
